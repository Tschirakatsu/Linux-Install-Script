...
install_app() {
  ...
  log "Processing $name..."
  if [[ -n "$fpkg" && 
        flatpak info "$fpkg" &>/dev/null ]]; then
...
install_ssh() {
  install_app "OpenSSH & UFW" "" openssh-server
  ufw --force enable >>"$VERBOSE_LOG" 2>&1
  ufw allow OpenSSH >>"$VERBOSE_LOG" 2>&1
}
...
install_portainer() {
  log "Checking Portainer..."
  if ! docker ps -a | grep -q portainer; then
    docker volume create portainer_data >>"$VERBOSE_LOG" 2>&1
    docker run -d -p 9000:9000 --name=portainer --restart=always \
      -v /var/run/docker.sock:/var/run/docker.sock -v portainer_data:/data portainer/portainer-ce >>"$VERBOSE_LOG" 2>&1
    log "Portainer installed"
  else
    log "Portainer already present"
  fi
}
...
check_amd_gpu() {
  if lspci | grep -i amd | grep -q vga && ! lsmod | grep -q amdgpu; then
    apt-get install -y linux-headers-$(uname -r) amdgpu-pro >>"$VERBOSE_LOG" 2>&1 && log "AMDGPU driver installed"; fi
}
check_ryzen_microcode() {
  if [[ ! -f /lib/firmware/amd-ucode/amd-ucode.bin ]]; then
    apt-get install -y amd64-microcode >>"$VERBOSE_LOG" 2>&1 && log "Ryzen microcode installed"; fi
}
...
# --- Hardening & tweaks ---
apply_hardening() {
  cat >/etc/sysctl.d/99-sysadmin.conf <<EOF
...
install_synth_shell(){ install_app "synth-shell" "" ""; git clone https://github.com/andresgongora/synth-shell.git /opt/synth-shell >>"$VERBOSE_LOG" 2>&1; bash /opt/synth-shell/setup.sh --install >>"$VERBOSE_LOG" 2>&1; }
install_hack_font(){ if ! fc-list | grep -q 'Hack Nerd Font'; then apt-get install -y unzip >>"$VERBOSE_LOG" 2>&1; mkdir -p /usr/local/share/fonts/NerdFonts; wget -O /tmp/Hack.zip https://github.com/ryanoasis/nerd-fonts/releases/download/v3.4.0/Hack.zip >>"$VERBOSE_LOG" 2>&1; unzip -o /tmp/Hack.zip -d /usr/local/share/fonts/NerdFonts >>"$VERBOSE_LOG" 2>&1; rm /tmp/Hack.zip; fc-cache -fv >>"$VERBOSE_LOG" 2>&1; fi; }
...
gaming_tasks(){ log "Gaming profile..."; install_spotify; install_steam; install_discord; install_lutris; install_easyeffects; install_zenbrowser; install_synth_shell; install_hack_font; install_lmsensors; install_fail2ban; setup_cli_tools; grant_docker_group; }
